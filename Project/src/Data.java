...
public class Data extends HttpServlet
{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			ResultSet aResultSet = stmt.executeQuery("SELECT * FROM tbl_cipher;");
			while(aResultSet.next()) {
				String[] str = {
					aResultSet.getString("id"),
					aResultSet.getString("encryptedTxt"),
					aResultSet.getString("shiftNo")
				};
				for (int i = 0; i < str.length-2; i++) {
					out.print("<data id= '" + str[0] + "' encryptedTxt= '" + str[1] + "' shiftNo= '" + str[2] + "'></data>" + "\n");
				}
			}
			out.println("</encrypt>");
		}
